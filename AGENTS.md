...
面向村民的极简AI提示词生成工具。3步流程：选类型→填信息→生成结果→选参考图。输出两版AI提示词（短视频分镜+海报图片），每版再分公域引流/私域转化两种，可直接粘贴到Sora/可灵/Midjourney/DALL-E等工具使用。第4步可选参考图（预设样图+自传），让AI生成的内容更贴近真实场景。
